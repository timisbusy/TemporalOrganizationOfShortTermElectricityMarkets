module LauraConvergenceMarketModel
...
function build_market_clearing!(m::Model, time_period::Int, marketresults, initialization::Dict{Symbol,Any}, data::Dict{Symbol,Any}, market::Dict{Symbol,Any})
    ...
    # TAtoLLD Change : use full dispatch at welfare, not just adjustments
    ...
    #=  TAtoLLD Change : no demand adjustments
    ...
    #=  TAtoLLD Change : generator upper and lower constraints
    ...
    m.ext[:constraints][:q_lower] = @constraint(
        m, [g in IG, t in OW],
        Qg_adj[g,t] >= -Qg_prev[(String(g),t)]
    )
    m.ext[:constraints][:q_upper] = @constraint(
        m, [g in IG, t in OW],
        Qg_adj[g,t] <= Q_gen[(String(g),t)] - Qg_prev[(String(g),t)]
    )
    ...
    if has_storage
        ...
        # TAtoLLD Change: remove storage end
...
# data - information about assets - prices, availability factors, etc # TODO: rename this config to match elsewhere
