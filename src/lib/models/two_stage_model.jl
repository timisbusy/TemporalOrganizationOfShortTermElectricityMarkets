...
# NOTE: This model does a first run with open charge/discharge for storage and then if a simultaneous charge/discharge operation occurs, restricts storage to only the higher of the two for that MTU in the second stage. It verifies that the objective value is the same in both cases and surfaces the objective value, and the MTUs constrained in a data frame which the market clearer will store for analysis.

# tolerance below which a Qch/Qdis value is treated as zero when checking for simultaneous charge+discharge -
# solver output can carry tiny nonzero noise on a variable that's really at its bound
const SIMULTANEOUS_CHARGE_DISCHARGE_TOLERANCE = 1e-6

# given stage-1 charge/discharge solutions (dicts keyed by MTU) and the optimization window,
# returns a DataFrame of MTUs where both are simultaneously nonzero (above tolerance), the
# stage-1 values, and which side ("charge"/"discharge") stage 2 should force to zero - the
# smaller of the two, so storage is restricted to only the higher of the two operations.
# a tie forces discharge to zero (keeps charge) as an arbitrary, deterministic tie-break.
function detect_simultaneous_charge_discharge(Qch_vals::Dict, Qdis_vals::Dict, OW, tol::Float64=SIMULTANEOUS_CHARGE_DISCHARGE_TOLERANCE)
    constrained = DataFrame(mtu=Int[], qch_stage1=Float64[], qdis_stage1=Float64[], forced_zero=String[])

    for t in OW
        qch = Qch_vals[t]
        qdis = Qdis_vals[t]
        if qch > tol && qdis > tol
            forced_zero = qdis > qch ? "charge" : "discharge"
            push!(constrained, (t, qch, qdis, forced_zero))
        end
    end
...
function build_market_clearing!(m::Model, time_period::Int, marketresults, initialization::Dict{Symbol,Any}, data::Dict{Symbol,Any}, market::Dict{Symbol,Any})
    ...
    if has_storage
        ...
        # two-stage restriction: for each MTU flagged by stage 1 as having simultaneous
        # charge and discharge, force the smaller of the two to zero so storage is restricted
        # to only the higher of the two operations there - set on m.ext by build() before this
        # runs, present only for the stage-2 model
        if haskey(m.ext, :two_stage_forced_zero)
            m.ext[:constraints][:two_stage_simultaneous_charge_discharge_restriction] = Dict{Int,ConstraintRef}()
            for (t, direction) in m.ext[:two_stage_forced_zero]
                m.ext[:constraints][:two_stage_simultaneous_charge_discharge_restriction][t] = direction == "charge" ? @constraint(m, Qch[t] == 0) : @constraint(m, Qdis[t] == 0)
            end
        end
    end
...
function build(time_period, marketresults, initialization, data, market)
    ...
    # Stage 1: solve with open (unconstrained) storage charge/discharge
    m1 = Model(select_optimizer(data))
    set_silent(m1)
    define_sets!(m1, time_period, marketresults, initialization, data, market)
    process_time_series_data!(m1, time_period, marketresults, initialization, data, market)
    process_parameters!(m1, time_period, marketresults, initialization, data, market)
    build_market_clearing!(m1, time_period, marketresults, initialization, data, market)
    optimize!(m1)

    if !m1.ext[:parameters][:has_storage]
        return m1
    end
    ...
    constrained = detect_simultaneous_charge_discharge(Qch1, Qdis1, OW)

    if nrow(constrained) == 0
        obj1 = objective_value(m1)
        m1.ext[:two_stage] = Dict(:objective_stage1 => obj1, :objective_stage2 => obj1, :constrained_mtus => constrained, :was_constrained => false)
        return m1
    end

    # Stage 2: re-solve with the smaller of charge/discharge forced to zero at each flagged MTU,
    # restricting storage to only the higher of the two operations there - build_market_clearing!
    # adds these constraints itself, since m.ext[:two_stage_forced_zero] is set below before it runs
    ...
    if !isapprox(obj1, obj2; atol=1e-4, rtol=1e-6)
        println("WARNING: TwoStageMarketModel objective mismatch at t=$time_period market=$(market[:name]): stage1=$obj1 stage2=$obj2")
    end
