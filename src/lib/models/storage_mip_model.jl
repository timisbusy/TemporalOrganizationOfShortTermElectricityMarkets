...
# NOTE: THIS IS A COPY OF LATEST_MODEL with storage charge/discharge exclusivity enforced
# exactly via a Big-M disjunctive constraint (a binary z[t] per MTU, in the spirit of
# `allow_only_one_direction_precise` from the Pyomo/AdOpT-NET0 Stor technology). This
# replaces the plain Morales-Espana LP cut (Hernandez, Helisto, Kiviluoma (2022) eq. 24)
# this model started as, because the LP-only cut can't give a hard guarantee: when
# storage_efficiency == 1 (free round trip), gross charge/discharge only enters the model
# through their net (Qdis[t] - Qch[t]), so an LP is free to pick any Qch[t] == Qdis[t] <=
# P_cap/2 at zero objective cost.
#
# This makes the market-clearing model a MIP, and MIPs don't have valid LP duals - dual.()
# on a solved MIP's energy_balance constraint is not a meaningful price. So build() below
# runs a mandatory two-solve pattern, unlike TwoStageMarketModel's *conditional* re-solve:
#   1. solve the MIP to decide, per MTU, which side (charge/discharge) the binary picked
#   2. rebuild the same model with the binary dropped and the losing side fixed to zero,
#      i.e. a pure LP with the MIP's dispatch decision baked in as fixed bounds
#   3. solve that LP and return it, so HelperModelResults.Prices() (dual.() on
#      energy_balance) works as it does for every other model in this codebase
# Step 2 is required on every clearing, even when stage 1 happened to already be exclusive,
# because a MIP solve never yields valid duals - only the fixed-bounds LP re-solve does.
...
function build(time_period, marketresults, initialization, data, market)
    ...
    # Stage 1: solve the MIP - the Big-M disjunctive constraint in build_market_clearing!
    # decides, per MTU, which side (charge/discharge) storage is allowed to use
    ...
    if !m1.ext[:parameters][:has_storage]
        # no storage, no binary was added - m1 is already a plain LP, its duals are valid
        return m1
    end
