...
function plot(marketresult, config, test_range, test_id)
    ...
    market_results = MarketDataStorage.GetFinalDispatchDecisionsForRange(marketresult,test_range)

    # Define consistent colors
    gen_colors = [:steelblue, :lightgreen, :red, :lightyellow, :coral, :orange]

    # Manual stacking order
    stack_order = marketresult.Results[1].AgentMap[Helpers.HelperModelResults.AGENT_GENERATOR]# ["Base", "Shoulder", "Peak", "Wind", "Solar"]
    ...
    total_demand = [sum(market_results[market_results.mtu .== t, d] for d in marketresult.Results[1].AgentMap[HelperModelResults.AGENT_DEMAND])[1,1] for t in test_range]
...
function plotCompare(market_results, config, test_range, test_id)
    ...
    # Manual stacking order
    stack_order = market_results[first(keys(market_results))].Results[1].AgentMap[Helpers.HelperModelResults.AGENT_GENERATOR]

    marker_shapes = [:circle,:star5,:rect,:cross,:diamond]
    ...
    for (marketConfiguration, marketResult) in market_results
        finalDispatchResults[marketConfiguration] = MarketDataStorage.GetFinalDispatchDecisionsForRange(marketResult,test_range)

        # Build matrix for areaplot (each row is a generator, each column is an mtu)
        stack_matrix = zeros(length(stack_order), length(test_range))
        for (i, g) in enumerate(stack_order)
            stack_matrix[i, :] = finalDispatchResults[marketConfiguration][:, g]
        end

        # Add storage discharge as another row
        discharge_vec = finalDispatchResults[marketConfiguration][:, "StorageDischarge"]
        if maximum(discharge_vec) > 0.1
            stack_matrix = vcat(stack_matrix, discharge_vec')
            labels = [stack_order; "Storage Discharge"]
        end
        ...
        # Calculate max y for limits
        total_demand = [sum(finalDispatchResults[marketConfiguration][finalDispatchResults[marketConfiguration].mtu .== t, d] for d in marketResult.Results[1].AgentMap[HelperModelResults.AGENT_DEMAND])[1,1] for t in test_range]
        charging_vec = finalDispatchResults[marketConfiguration][:, "StorageCharge"]
        max_y = maximum(total_demand .+ charging_vec) * 1.6
        maxY = maximum([max_y, maxY])
        stackMatrices[marketConfiguration] = stack_matrix
    end
    ...
    markerIter = 1
    for (marketConfiguration, stack_matrix) in stackMatrices
        ...
        for i in 1:size(stack_matrix, 1)
            if i == 1
                Plots.plot!(p3, test_range, stack_matrix[i, :],
                     label="$(marketConfiguration): $(labels[i])" , 
                    fillrange=0, color=gen_colors[i], alpha=0.6, linewidth=1)
            else
                cumsum_prev = vec(sum(stack_matrix[1:i-1, :], dims=1))
                cumsum_curr = vec(sum(stack_matrix[1:i, :], dims=1))
                Plots.plot!(p3, test_range, cumsum_curr,
                     label="$(marketConfiguration): $(labels[i])",
                    fillrange=cumsum_prev, color=gen_colors[i], alpha=0.6, linewidth=1)
            end
        end
    ...
    for (marketConfiguration, stack_matrix) in stackMatrices
        ...
        plot_arr[markerIter] = Plots.plot(xlabel="MTU", ylabel="Dispatched Production (MWh)",
            title="Generation & Demand Stack for $marketConfiguration",
            legend=:topright,
            ylims=(0, maxY),size=(1200,1200))
        # Stack manually using areaplot with seriestype
        for i in 1:size(stack_matrix, 1)
        ...
        savefig(plot_arr[markerIter], "results/$(test_id)/generation_stack_$(test_id)_$(marketConfiguration).png")
    end
    return 
end
